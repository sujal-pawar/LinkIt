...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    // The canvas is created here (not in JSX) on purpose. dispose() calls
    // renderer.forceContextLoss(), and a canvas whose context was force-lost
    // can never hand out a new WebGL context. React 18 StrictMode (dev) runs
    // effect -> cleanup -> effect on the SAME DOM node, so re-using a JSX
    // canvas made the second setup crash with "Cannot read properties of null".
    // A fresh canvas per effect run avoids that.
    ...
    try {
      ...
      canvas.remove();
      throw err; // surfaces to the parent ErrorBoundary (e.g. WebGL unavailable)
    }
